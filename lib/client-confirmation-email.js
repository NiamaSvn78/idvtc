/**
 * Email confirmation client après paiement Stripe (QR inline CID, un QR par course).
 */
...
function prestBlocksHtml(prestDetails, prestQrs, lang) {
  ...
  return prestDetails
    ...
    <div style="border-top:1px solid #e8e0d0;padding-top:16px;margin-bottom:20px">
      ...
      ${
        prestQrs[i]
          ? `<div style="background:#080808;border:1px solid #c9a96e;border-radius:3px;padding:16px;text-align:center;margin-bottom:8px">
          <div style="font-size:.6rem;color:#9a9185;letter-spacing:.15em;text-transform:uppercase;margin-bottom:6px">QR code — ${label} ${i + 2}</div>
          <img src="${prestQrs[i]}" width="160" height="160" alt="QR ${label} ${i + 2}" style="display:block;margin:0 auto;border:3px solid #fff;border-radius:2px"/>
        </div>`
          : ''
      }
    </div>`
...
function createClientConfirmationMailer(opts) {
  ...
  function buildClientConfirmationHtml(r, qrCid, prestDetails = [], prestQrs = []) {
    if (r.lang === 'en') return buildClientConfirmationHtmlEN(r, qrCid, prestDetails, prestQrs);
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:4px;overflow:hidden;border:1px solid #e8e8e8">
  ...
  <tr><td style="padding:28px 32px 20px">
    ...
    <div style="background:#080808;border:1px solid #c9a96e;border-radius:3px;padding:22px;text-align:center;margin-bottom:16px">
      <div style="font-size:.63rem;color:#9a9185;letter-spacing:.2em;text-transform:uppercase;margin-bottom:6px">${qrTitle}</div>
      ...
      <img src="${qrCid}" width="200" height="200" alt="QR code réservation IsmaDrive" style="display:block;margin:0 auto;border:4px solid #fff;border-radius:2px"/>
      <div style="margin-top:10px;font-size:.72rem;color:#777">Sauvegardez ce mail ou faites une capture d'écran.</div>
    </div>
    ${prestBlocksHtml(prestDetails, prestQrs, 'fr')}
  ...
  function buildClientConfirmationHtmlEN(r, qrCid, prestDetails = [], prestQrs = []) {
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:4px;overflow:hidden;border:1px solid #e8e8e8">
  ...
  <tr><td style="padding:28px 32px 20px">
    ...
    <div style="background:#080808;border:1px solid #c9a96e;border-radius:3px;padding:22px;text-align:center;margin-bottom:16px">
      ...
      <img src="${qrCid}" width="200" height="200" alt="IsmaDrive booking QR code" style="display:block;margin:0 auto;border:4px solid #fff;border-radius:2px"/>
      <div style="margin-top:10px;font-size:.72rem;color:#777">Save this email or take a screenshot.</div>
    </div>
    ${prestBlocksHtml(prestDetails, prestQrs, 'en')}
    <div style="text-align:center;margin-bottom:12px">
  ...
  async function sendClientConfirmationEmail(r) {
    ...
    const qrBuffer = await QRCode.toBuffer(reservationUrl(appUrl, r.id, 1), {
      width: 260,
      margin: 2,
    ...
    const prestQrBuffers = await Promise.all(
      prestDetails.map((_, i) =>
        QRCode.toBuffer(reservationUrl(appUrl, r.id, i + 2), {
          width: 200,
          margin: 2,
          errorCorrectionLevel: 'M',
          color: { dark: '#000000', light: '#ffffff' },
    ...
    const payload = { from, to: email, subject, html, attachments };
