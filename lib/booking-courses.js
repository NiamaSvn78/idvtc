...
/** Notes libres sans les blocs [Prestation N:…] (autres courses). */
function notesFreeTextOnly(notes) {
  if (!notes) return '';
  return notes.replace(/\[Prestation \d+:[^\]]*\]/g, '').replace(/\s+/g, ' ').trim();
}

function applyCourseToReservation(r, courseIdx) {
  if (!r || !courseIdx || courseIdx <= 1) {
    return r ? { ...r, _courseIdx: 1, _courseLabel: 'Course 1' } : r;
  }
